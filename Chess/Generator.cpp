...
namespace Hopper {
	...
	int Board::removeIllegal(Move* m, int cmove) {
		...
		for (int i = 0; i < threatened[!turn][kpos[turn]]; ++i) {
			if (abs(grid[attackers[!turn][i][kpos[turn]]]) <= W_KNIGHT) { checktype = BOARD_LEAP; }
			else if (NSslide(attackers[!turn][i][kpos[turn]], kpos[turn])) { checktype = (attackers[!turn][i][kpos[turn]] > kpos[turn]) ? BOARD_NORTH : BOARD_SOUTH; }
			else if (EWslide(attackers[!turn][i][kpos[turn]], kpos[turn])) { checktype = (attackers[!turn][i][kpos[turn]] > kpos[turn]) ? BOARD_WEST : BOARD_EAST; }
			else if (NESWslide(attackers[!turn][i][kpos[turn]], kpos[turn])) { checktype = (attackers[!turn][i][kpos[turn]] > kpos[turn]) ? BOARD_NORTHEAST : BOARD_SOUTHWEST; }
			else { checktype = (attackers[!turn][i][kpos[turn]] > kpos[turn]) ? BOARD_NORTHWEST : BOARD_SOUTHEAST; }
			for (int j = 0; j < cmove; ++j) {
				to = m[j].getTo();
				from = m[j].getFrom();
				if (from != kpos[turn]) {
					if (checktype != BOARD_LEAP && ((to - attackers[!turn][i][kpos[turn]]) % checktype || (to < attackers[!turn][i][kpos[turn]] && to < kpos[turn]) || (to > attackers[!turn][i][kpos[turn]] && to > kpos[turn]))) { m[j--] = m[--cmove]; }
					else if (checktype == BOARD_LEAP && to != attackers[!turn][i][kpos[turn]] && m[j].getFlags() != ENPASSANT) { m[j--] = m[--cmove]; }
				}
				else if (checktype != BOARD_LEAP && to - from == checktype) { m[j--] = m[--cmove]; }
			}
		}
		return cmove;
	}
	...
	int Board::pieceCapMoves(Move* m, int from) {//generates all pseudo legal capture moves for one piece
		...
		case W_KING:
			if ((from + BOARD_SOUTHEAST) % WIDTH > from % WIDTH && from < 55 && !threatened[!turn][from + BOARD_SOUTHEAST] && ((turn && grid[from + BOARD_SOUTHEAST] < 0) || (!turn && grid[from + BOARD_SOUTHEAST] > 0))) { m[cmove++] = Move(from, from + BOARD_SOUTHEAST, CAPTURE); }
			if ((from + BOARD_EAST) % WIDTH > from % WIDTH && !threatened[!turn][from + BOARD_EAST] && ((turn && grid[from + BOARD_EAST] < 0) || (!turn && grid[from + BOARD_EAST] > 0))) { m[cmove++] = Move(from, from + BOARD_EAST, CAPTURE); }
			if ((from + BOARD_NORTHWEST) % WIDTH < from % WIDTH && from > 8 && !threatened[!turn][from + BOARD_NORTHWEST] && ((turn && grid[from + BOARD_NORTHWEST] < 0) || (!turn && grid[from + BOARD_NORTHWEST] > 0))) { m[cmove++] = Move(from, from + BOARD_NORTHWEST, CAPTURE); }
			if ((from + BOARD_WEST) % WIDTH < from % WIDTH && from > 0 && !threatened[!turn][from + BOARD_WEST] && ((turn && grid[from + BOARD_WEST] < 0) || (!turn && grid[from + BOARD_WEST] > 0))) { m[cmove++] = Move(from, from + BOARD_WEST, CAPTURE); }
			if ((from + BOARD_SOUTHWEST) % WIDTH < from % WIDTH && from < 57 && !threatened[!turn][from + BOARD_SOUTHWEST] && ((turn && grid[from + BOARD_SOUTHWEST] < 0) || (!turn && grid[from + BOARD_SOUTHWEST] > 0))) { m[cmove++] = Move(from, from + BOARD_SOUTHWEST, CAPTURE); }
			if (from < 56 && !threatened[!turn][from + BOARD_SOUTH] && ((turn && grid[from + BOARD_SOUTH] < 0) || (!turn && grid[from + BOARD_SOUTH] > 0))) { m[cmove++] = Move(from, from + BOARD_SOUTH, CAPTURE); }
			if ((from + BOARD_NORTHEAST) % WIDTH > from % WIDTH && from > 6 && !threatened[!turn][from + BOARD_NORTHEAST] && ((turn && grid[from + BOARD_NORTHEAST] < 0) || (!turn && grid[from + BOARD_NORTHEAST] > 0))) { m[cmove++] = Move(from, from + BOARD_NORTHEAST, CAPTURE); }
			if (from > 7 && !threatened[!turn][from + BOARD_NORTH] && ((turn && grid[from + BOARD_NORTH] < 0) || (!turn && grid[from + BOARD_NORTH] > 0))) { m[cmove++] = Move(from, from + BOARD_NORTH, CAPTURE); }
			return cmove;
		case W_PAWN:
			i = (turn) ? BOARD_NORTH : BOARD_SOUTH;
			if (from % WIDTH && ((turn && grid[from + i + BOARD_WEST] < 0) || (!turn && grid[from + i + BOARD_WEST] > 0))) {
				if ((turn && from > 15) || (!turn && from < 48)) { m[cmove++] = Move(from, from + i + BOARD_WEST, CAPTURE); }
				else {
			...
			if (from % WIDTH != 7 && ((turn && grid[from + i + BOARD_EAST] < 0) || (!turn && grid[from + i + BOARD_EAST] > 0))) {
				if ((turn && from > 15) || (!turn && from < 48)) { m[cmove++] = Move(from, from + i + BOARD_EAST, CAPTURE); }
				else {
			...
			if (mHist[cturn - 1].getFlags() == DOUBLEPUSH && ((mHist[cturn - 1].getTo() == from + BOARD_EAST && from % WIDTH != 7) || (mHist[cturn - 1].getTo() == from + BOARD_WEST && from % WIDTH))) { m[cmove++] = Move(from, mHist[cturn - 1].getTo() + i, ENPASSANT); }
			return cmove;
		case W_KNIGHT:
		...
		case W_ROOK:
			...
			if (abs(grid[from]) != W_QUEEN) { return cmove; }
		case W_BISHOP:
	...
	int Board::pieceNonCapMoves(Move* m, int from) {//generates all pseudo legal non capture moves for one piece
		...
		case W_KING:
			...
			if ((from + BOARD_SOUTHEAST) % WIDTH > from % WIDTH && from < 55 && !threatened[!turn][from + BOARD_SOUTHEAST] && !grid[from + BOARD_SOUTHEAST]) { m[cmove++] = Move(from, from + BOARD_SOUTHEAST, STANDARD); }
			if ((from + BOARD_EAST) % WIDTH > from % WIDTH && !threatened[!turn][from + BOARD_EAST] && !grid[from + BOARD_EAST]) { m[cmove++] = Move(from, from + BOARD_EAST, STANDARD); }
			if ((from + BOARD_NORTHWEST) % WIDTH < from % WIDTH && from > 8 && !threatened[!turn][from + BOARD_NORTHWEST] && !grid[from + BOARD_NORTHWEST]) { m[cmove++] = Move(from, from + BOARD_NORTHWEST, STANDARD); }
			if ((from + BOARD_WEST) % WIDTH < from % WIDTH && from > 0 && !threatened[!turn][from + BOARD_WEST] && !grid[from + BOARD_WEST]) { m[cmove++] = Move(from, from + BOARD_WEST, STANDARD); }
			if ((from + BOARD_SOUTHWEST) % WIDTH < from % WIDTH && from < 57 && !threatened[!turn][from + BOARD_SOUTHWEST] && !grid[from + BOARD_SOUTHWEST]) { m[cmove++] = Move(from, from + BOARD_SOUTHWEST, STANDARD); }
			if (from < 56 && !threatened[!turn][from + BOARD_SOUTH] && !grid[from + BOARD_SOUTH]) { m[cmove++] = Move(from, from + BOARD_SOUTH, STANDARD); }
			if ((from + BOARD_NORTHEAST) % WIDTH > from % WIDTH && from > 6 && !threatened[!turn][from + BOARD_NORTHEAST] && !grid[from + BOARD_NORTHEAST]) { m[cmove++] = Move(from, from + BOARD_NORTHEAST, STANDARD); }
			if (from > 7 && !threatened[!turn][from + BOARD_NORTH] && !grid[from + BOARD_NORTH]) { m[cmove++] = Move(from, from + BOARD_NORTH, STANDARD); }
			return cmove;
		case W_PAWN:
			i = (turn) ? BOARD_NORTH : BOARD_SOUTH;
			if (!grid[from + i]) {
	...
	int Board::pieceMoves(Move* m, int from) {//generates all pseudo legal moves for one piece
		...
		case W_KING:
			...
			if ((from + BOARD_SOUTHEAST) % WIDTH > from % WIDTH && from < 55) {
				if (!threatened[!turn][from + BOARD_SOUTHEAST] && ((turn && grid[from + BOARD_SOUTHEAST] <= 0) || (!turn && grid[from + BOARD_SOUTHEAST] >= 0))) {
					if (!grid[from + BOARD_SOUTHEAST]) { m[cmove++] = Move(from, from + BOARD_SOUTHEAST, STANDARD); }
					else { m[cmove++] = Move(from, from + BOARD_SOUTHEAST, CAPTURE); }
				}
			}
			if ((from + BOARD_EAST) % WIDTH > from % WIDTH) {
				if (!threatened[!turn][from + BOARD_EAST] && ((turn && grid[from + BOARD_EAST] <= 0) || (!turn && grid[from + BOARD_EAST] >= 0))) {
					if (!grid[from + BOARD_EAST]) { m[cmove++] = Move(from, from + BOARD_EAST, STANDARD); }
					else { m[cmove++] = Move(from, from + BOARD_EAST, CAPTURE); }
				}
			}
			if ((from + BOARD_NORTHWEST) % WIDTH < from % WIDTH && from > 8) {
				if (!threatened[!turn][from + BOARD_NORTHWEST] && ((turn && grid[from + BOARD_NORTHWEST] <= 0) || (!turn && grid[from + BOARD_NORTHWEST] >= 0))) {
					if (!grid[from + BOARD_NORTHWEST]) { m[cmove++] = Move(from, from + BOARD_NORTHWEST, STANDARD); }
					else { m[cmove++] = Move(from, from + BOARD_NORTHWEST, CAPTURE); }
				}
			}
			if ((from + BOARD_WEST) % WIDTH < from % WIDTH && from > 0) {
				if (!threatened[!turn][from + BOARD_WEST] && ((turn && grid[from + BOARD_WEST] <= 0) || (!turn && grid[from + BOARD_WEST] >= 0))) {
					if (!grid[from + BOARD_WEST]) { m[cmove++] = Move(from, from + BOARD_WEST, STANDARD); }
					else { m[cmove++] = Move(from, from + BOARD_WEST, CAPTURE); }
				}
			}
			if ((from + BOARD_SOUTHWEST) % WIDTH < from % WIDTH && from < 57) {
				if (!threatened[!turn][from + BOARD_SOUTHWEST] && ((turn && grid[from + BOARD_SOUTHWEST] <= 0) || (!turn && grid[from + BOARD_SOUTHWEST] >= 0))) {
					if (!grid[from + BOARD_SOUTHWEST]) { m[cmove++] = Move(from, from + BOARD_SOUTHWEST, STANDARD); }
					else { m[cmove++] = Move(from, from + BOARD_SOUTHWEST, CAPTURE); }
				}
			}
			if (from < 56) {
				if (!threatened[!turn][from + BOARD_SOUTH] && ((turn && grid[from + BOARD_SOUTH] <= 0) || (!turn && grid[from + BOARD_SOUTH] >= 0))) {
					if (!grid[from + BOARD_SOUTH]) { m[cmove++] = Move(from, from + BOARD_SOUTH, STANDARD); }
					else { m[cmove++] = Move(from, from + BOARD_SOUTH, CAPTURE); }
				}
			}
			if ((from + BOARD_NORTHEAST) % WIDTH > from % WIDTH && from > 6) {
				if (!threatened[!turn][from + BOARD_NORTHEAST] && ((turn && grid[from + BOARD_NORTHEAST] <= 0) || (!turn && grid[from + BOARD_NORTHEAST] >= 0))) {
					if (!grid[from + BOARD_NORTHEAST]) { m[cmove++] = Move(from, from + BOARD_NORTHEAST, STANDARD); }
					else { m[cmove++] = Move(from, from + BOARD_NORTHEAST, CAPTURE); }
				}
			}
			if (from > 7) {
				if (!threatened[!turn][from + BOARD_NORTH] && ((turn && grid[from + BOARD_NORTH] <= 0) || (!turn && grid[from + BOARD_NORTH] >= 0))) {
					if (!grid[from + BOARD_NORTH]) { m[cmove++] = Move(from, from + BOARD_NORTH, STANDARD); }
					else { m[cmove++] = Move(from, from + BOARD_NORTH, CAPTURE); }
				}
			}
			return cmove;
		case W_PAWN:
